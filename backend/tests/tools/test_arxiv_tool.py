...
from labora.tools import ArxivTool
import arxiv
...
class TestArxivTool:
    ...
    def test_search_returns_correct_fields(self, tool):
        """测试搜索返回正确的字段"""
        # 使用真实 API 搜索（限制结果数量）
        results = tool.search("attention mechanism", max_results=3)
    ...
    def test_search_attention_is_all_you_need(self, tool):
        """测试搜索 'Attention Is All You Need' 论文"""
        # 使用更精确的搜索（包含作者）
        results = tool.search("Attention Is All You Need Vaswani", max_results=10)
    ...
    def test_search_with_max_results(self, tool):
        """测试 max_results 参数"""
        results = tool.search("transformer", max_results=5)
        assert len(results) <= 5

    def test_get_paper_by_id(self, tool):
        """测试根据 ID 获取论文"""
        # Attention Is All You Need 的 ArXiv ID
        paper = tool.get_paper_by_id("1706.03762")
    ...
    def test_get_nonexistent_paper(self, tool):
        """测试获取不存在的论文"""
        paper = tool.get_paper_by_id("9999.99999")
        assert paper is None

    @patch("arxiv.Client")
    def test_search_network_error(self, mock_client_class, tool):
        """测试网络错误处理"""
        mock_client = Mock()
    ...
    def test_search_response_time(self, tool):
        """测试搜索响应时间 < 3 秒"""
        import time
    ...
    def test_paper_format(self, tool):
        """测试论文格式化"""
        results = tool.search("BERT", max_results=1)
        paper = results[0]
