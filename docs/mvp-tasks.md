...
**验收标准**：
- [ ] `cd backend && uv run python main.py` 启动成功，`GET /health` 返回 `{"status": "ok"}`
- [ ] `cd frontend && npm run dev` 启动成功，浏览器访问 `localhost:5173` 看到应用页面
- [ ] `cd desktop && npm run dev` 启动成功，Electron 窗口加载前端页面
- [ ] Electron 启动时自动拉起 Python 进程，关闭时自动终止
- [ ] 前端能通过 `fetch` 调用后端 `/health` 并显示状态
...
**验收标准**：
- [ ] `InMemoryCache` 通过 `IShortTermMemory` 接口的所有单元测试（set/get/delete/TTL）
- [ ] `SQLiteMemory` 通过 `ILongTermMemory` 接口的所有单元测试（add_paper/search_papers/add_note）
- [ ] `MemoryManager` 可通过配置文件切换后端，切换后单元测试全部通过
- [ ] `MemoryManager.save_paper_analysis()` 调用后，数据可通过 `get_paper()` 正确取回
- [ ] 代码中没有对 SQLite/内存直接操作，全部通过接口访问
...
**验收标准**：
- [ ] `arxiv_tool.search("attention mechanism", max_results=10)` 在 3 秒内返回结果
- [ ] 返回结果包含 `title`、`abstract`、`authors`、`arxiv_id`、`year` 字段
- [ ] 搜索 "Attention Is All You Need" 时，原始论文出现在前 3 条结果中
- [ ] 网络不可用时抛出明确的异常，而不是静默失败
- [ ] 有对应的单元测试（使用 mock HTTP 响应，不依赖真实网络）
...
- [ ] 用户回答问题后，`MemorySaver` 能正确恢复工作流状态，不丢失上下文
